...
class SubBassProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    this.nparams = { drop: 0, droptm: 0, atk: 0, click: 0, phase: 0, rdec: 0, racc: 0 };
    this.glideSec = 0;
    ...
    for (let base = 0; base < n128; base += BLK) {
      ...
      while (this.queue.len && this.queue.headAt() <= at) {
        ...
        if (ev.off) this.noteOff(ev.id);
          ...
          // The note's snapshot of the a-rate params, written into one reused
          // object rather than a fresh literal per note: noteOn reads it and
          // keeps nothing, so this is the queue's scratch event all over again.
          const np = this.nparams;
          np.drop   = P.drop.length   > 1 ? P.drop[i]   : P.drop[0];
          np.droptm = P.droptm.length > 1 ? P.droptm[i] : P.droptm[0];
          np.atk    = P.atk.length    > 1 ? P.atk[i]    : P.atk[0];
          np.click  = P.click.length  > 1 ? P.click[i]  : P.click[0];
          np.phase  = P.phase.length  > 1 ? P.phase[i]  : P.phase[0];
          np.rdec   = P.rdec.length   > 1 ? P.rdec[i]   : P.rdec[0];
          np.racc   = P.racc.length   > 1 ? P.racc[i]   : P.racc[0];
          this.noteOn(ev, np);
        }
      }
