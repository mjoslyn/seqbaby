// Tests for the Strudel / Tidal bridge: the mini-notation's timing
...
// the notation means what it means in Strudel and Tidal (where an onset lands,
...
test("notes: Strudel spells middle C c4, Tidal counts from it", () => {
  ...
  assert.equal(S.noteNumber("c5", "tidal"), 60);
  assert.equal(S.noteNumber("e", "tidal"), 64);
  assert.equal(S.noteNumber(7, "tidal"), 67);
  assert.equal(S.noteNumber("cs5", "tidal"), 61);
  assert.equal(S.midiName(63), "eb4");
  assert.equal(S.midiName(63, "tidal"), "ef5");
});
...
test("dialects are told apart", () => {
  assert.equal(S.detectDialect(`d1 $ s "bd"`), "tidal");
  assert.equal(S.detectDialect(`setcps (120/60/4)\nd1 $ sound "bd"`), "tidal");
  assert.equal(S.detectDialect(`$: s("bd")`), "strudel");
  assert.equal(S.detectDialect(`s("bd sd").fast(2)`), "strudel");
});
...
test("Tidal: d1 $, #, $ chains, scale, signals", () => {
  const { song, dialect, warnings } = S.codeToSong(`setcps (130/60/4)
d1 $ sound "bd*2 [~ bd] sn" # gain 1.1
d2 $ every 4 (fast 2) $ n "0 .. 7" # s "superpiano" # lpf 1200
d3 $ note (scale "minor" "0 2 4") # s "supersaw"
  # room 0.3
d4 $ s "hh*8" # lpf (slow 4 $ range 200 4000 sine)`);
  assert.equal(dialect, "tidal");
  assert.equal(song.bpm, 130);
  assert.deepEqual(song.tracks.map(t => t.name), ["d1 bd", "d1 sn", "d2", "d3", "d4"]);
  assert.deepEqual([...steps(track(song, "d1 bd"))].flatMap((c, i) => c === "1" ? [i] : []), [0, 3, 8]);
  assert.deepEqual(track(song, "d2").patterns[0].notes.filter(n => n != null), [60, 61, 62, 63, 64, 65, 66, 67]);
  assert.deepEqual(track(song, "d3").patterns[0].notes.filter(n => n != null), [60, 63, 67]);
  assert.equal(track(song, "d3").fxConfig.reverb.wet, 0.3);
  assert.equal(track(song, "d4").lfoConfig.cutoff.div, 16);
  assert.ok(warnings.some(w => /every/.test(w)));
});
...
test("a song round-trips through Strudel and Tidal code", () => {
...
keys: chord("<Cm7 G7>").voicing().s("gm_epiano1").room(0.4)`;
  ...
  for (const dialect of ["strudel", "tidal"]) {
    const { code } = S.sessionToCode(a, { dialect });
    const b = S.codeToSong(code, { dialect }).song;
