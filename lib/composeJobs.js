...
/** Which usage bucket a job counts against: the account that started it, or
 *  -- with no account -- the key it will be run on. */
const bucketFor = ({ userId, keyHash }) =>
  userId ? `users/${userId}` : keyHash ? `keys/${keyHash}` : null;
 ...
 */
export async function createJob({ userId, keyHash, message, history, session, model }) {
  const bucket = bucketFor({ userId, keyHash });
